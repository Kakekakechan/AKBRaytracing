...
def psf_calc(matrixWave2_Corrected, grid_H, grid_V, defocusWave):
    ...
    if option_energy == 'EUV':
        wavelength_m = 13.5e-9
    elif option_energy == 'hardXray':
        wavelength_m = 1.24e-10
    elif option_energy == 'softXray':
        wavelength_m = 1.24e-9
    ...
    pupil_dx_m = np.abs(grid_H[0, 1] - grid_H[0, 0])
    pupil_dy_m = np.abs(grid_V[1, 0] - grid_V[0, 0])
    print('pupil_dx_m',pupil_dx_m)
    print('pupil_dy_m',pupil_dy_m)
    ...
    psf, x_im, y_im = compute_psf_fft(opd, amp, wavelength_m, pupil_dx_m, focal_length_m, pad_factor=16, window=None,pupil_dy_m=pupil_dy_m)
    # 範囲指定
    if option_energy == 'EUV':
    ...
    plt.imshow(psf_trimmed,
               ...
               origin='lower',
               cmap='inferno',
               norm=LogNorm(vmin=1e-11, vmax=1.0))
    plt.xlabel('x on sensor [m]')
    plt.ylabel('y on sensor [m]')
    plt.title('PSF (log, trimmed)')
    plt.colorbar()

    plt.savefig(os.path.join(directory_name, 'PSF_log.png'), transparent=True, dpi=300)
    ...
    plt.title('psf y')
    plt.xlim(y_min*1e9, y_max*1e9)
    plt.ylim(-0.005, 1.005)
    plt.savefig(os.path.join(directory_name, 'PSF_y_section.png'), transparent=True, dpi=300)

    np.save(os.path.join(directory_name, 'psf.npy'), psf)
    np.save(os.path.join(directory_name, 'psf_x.npy'), x_im)
    np.save(os.path.join(directory_name, 'psf_y.npy'), y_im)
    ############################################
    # plt.show()
...
if option_wolter_3_1:
    def plot_result_debug(params,option,source_shift=[0.,0.,0.],option_tilt = True,option_legendre=False, angular_shift = [0.,0.],option_save=True):
        ...
        if True:
            ...
            def plot_hyperbola(a, b, h=0.0, k=0.0, orientation='horizontal', tmax=2.0, n=1000, ax=None, **plot_kwargs):
                """
                orientation: 'horizontal' -> (x-h)^2/a^2 - (y-k)^2/b^2 = 1
                            'vertical'   -> (y-k)^2/a^2 - (x-h)^2/b^2 = 1
                """
                t = np.linspace(-tmax, tmax, n)  # パラメトリック: cosh/sinh で2枝を描く
                if orientation == 'horizontal':
                    x1 = h + a * np.cosh(t); y1 = k + b * np.sinh(t)
                    x2 = h - a * np.cosh(t); y2 = k - b * np.sinh(t)
                else:  # vertical
                    x1 = h + b * np.sinh(t); y1 = k + a * np.cosh(t)
                    x2 = h - b * np.sinh(t); y2 = k - a * np.cosh(t)

                if ax is None:
                    fig, ax = plt.subplots()
                ax.plot(x1, y1, **plot_kwargs)
                ax.plot(x2, y2, **plot_kwargs)
                # ax.set_aspect('equal', adjustable='box')
                # ax.set_xlabel('x'); ax.set_ylabel('y')
                return ax
            ...
            plot_ellipse(a_ell_v, b_ell_v, h = org_hyp_v*2 + org_ell_v, ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='k')
            plt.savefig('ell.png', dpi=300,transparent=True)
            plt.show()
            fig, ax = plt.subplots(figsize=(6,6))
            ...
            plt.savefig('fig_wolter3.png', dpi=300,transparent=True)
            plt.show()

            fig, ax = plt.subplots(figsize=(6,6))
            plot_ellipse(a_ell_h, b_ell_h, h = org_ell_h, ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='k')
            print('a_hyp_h',a_hyp_h)
            print('b_hyp_h',b_hyp_h)
            plot_hyperbola(a_hyp_h, b_hyp_h, h = org_ell_h*2 - org_hyp_h, orientation='horizontal',tmax=3, linewidth=0.5, ax=ax, label='Hyperbola', linestyle='--',color='k')

            ax.scatter([org_ell_h*2,-org_hyp_h*2+org_ell_h*2], [0,0], color='red',s=0.2)  # 焦点
            ax.set_xlim([145.95, 146.25])
            ax.set_ylim([-0.15, 0.15])

            # 目安の中心と軸
            # ax.plot(h, k, 'o', label='Center (h,k)')
            # ax.axvline(h, color='gray', alpha=0.3)
            # ax.axhline(k, color='gray', alpha=0.3)

            plt.savefig('fig_wolter1.png', dpi=300,transparent=True)
            plt.show()
            ...
            if option == 'sep' or option == 'wave' or option == 'ray' or option == 'ray_wave':
                ...
                if option == 'ray':
                    ...
                    if True:
                        ...
                        # plt.show()
                        # ### 光線1st to 2nd
                        for i in range(0, ray_num**2, round(ray_num**2/100)):
                        ...
                        # 光線2nd to 3rd
                        for i in range(0, ray_num**2, round(ray_num**2/100)):
                            ax.plot([vmirr_ell[0,i], vmirr_hyp[0,i]], [vmirr_ell[1,i], vmirr_hyp[1,i]], [vmirr_ell[2,i], vmirr_hyp[2,i]], color="#271558", alpha=0.7,linewidth=1)
                        # 光線3rd to 4th
                        for i in range(0, ray_num**2, round(ray_num**2/100)):
                            ax.plot([vmirr_ell[0,i], hmirr_ell[0,i]], [vmirr_ell[1,i], hmirr_ell[1,i]], [vmirr_ell[2,i], hmirr_ell[2,i]], color='#271558', alpha=0.7,linewidth=1)
                        # 光線4th to fcs
                        for i in range(0, ray_num**2, round(ray_num**2/100)):
                            ax.plot([hmirr_ell[0,i], hmirr_hyp[0,i]], [hmirr_ell[1,i], hmirr_hyp[1,i]], [hmirr_ell[2,i], hmirr_hyp[2,i]], color='#271558', alpha=0.7,linewidth=1)
                        for i in range(0, ray_num**2, round(ray_num**2/100)):
                            detcenter_edge=detcenter + (hmirr_hyp - detcenter)/(hmirr_hyp[0,0] - detcenter[0,0]) *250e-3
                            # detcenter_edge=detcenter.copy()
                            ax.plot([hmirr_hyp[0,i], detcenter_edge[0,i]], [hmirr_hyp[1,i], detcenter_edge[1,i]], [hmirr_hyp[2,i], detcenter_edge[2,i]], color='#271558', alpha=0.7,linewidth=1)
            ...
            if option == 'sep' or option == 'wave' or option == 'ray' or option == 'ray_wave':
                ...
                if option == 'ray':
                    if True:
                        ...
                        face_define2(vmirr_hyp, direction="V", positive=False)
                        face_define2(vmirr_ell, direction="V", positive=True)
                        face_define2(hmirr_hyp, direction="H", positive=False)
                        face_define2(hmirr_ell, direction="H", positive=True)
                        # plt.show()
                        # ### 光線1st to 2nd
                        ...
                        # 光線2nd to 3rd
                        for i in range(0, ray_num**2, round(ray_num**2/20)):
                            ax.plot([vmirr_ell[0,i], vmirr_hyp[0,i]], [vmirr_ell[1,i], vmirr_hyp[1,i]], [vmirr_ell[2,i], vmirr_hyp[2,i]], color="#271558", alpha=0.3,linewidth=1)
                        # 光線3rd to 4th
                        for i in range(0, ray_num**2, round(ray_num**2/20)):
                            ax.plot([hmirr_hyp[0,i], vmirr_ell[0,i]], [hmirr_hyp[1,i], vmirr_ell[1,i]], [hmirr_hyp[2,i], vmirr_ell[2,i]], color='#271558', alpha=0.3,linewidth=1)
                        # 光線4th to fcs
                        for i in range(0, ray_num**2, round(ray_num**2/20)):
                            ax.plot([hmirr_ell[0,i], hmirr_hyp[0,i]], [hmirr_ell[1,i], hmirr_hyp[1,i]], [hmirr_ell[2,i], hmirr_hyp[2,i]], color='#271558', alpha=0.3,linewidth=1)
                        for i in range(0, ray_num**2, round(ray_num**2/20)):
    ...
    def plot_result_debug(params,option,source_shift=[0.,0.,0.],option_legendre=False):
        ...
        if False:
            ...
            fig, ax = plt.subplots(figsize=(6,6))
            plot_ellipse(a_ell_v, b_ell_v, h = org_hyp_v*2 + org_ell_v, ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='y')
            ...
            ax.scatter([org_hyp_v*2,org_hyp_v*2+org_ell_v*2], [0,0], color='red',s=0.2)  # 焦点
            ax.set_xlim([145.8, 146.9])
            ax.set_ylim([-0.55, 0.55])
            plt.savefig('fig_wolter3_V.png', dpi=300,transparent=True)
            plt.show()

            # fig, ax = plt.subplots(figsize=(6,6))
            # plot_ellipse(a_ell_h, b_ell_h, h = org_ell_h, ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='y')
            # print('a_hyp_h',a_hyp_h)
            # print('b_hyp_h',b_hyp_h)
            # plot_hyperbola(a_hyp_h, b_hyp_h, h = org_ell_h*2 - org_hyp_h, orientation='horizontal',tmax=3, linewidth=0.5, ax=ax, label='Hyperbola', linestyle='--',color='y')
            ...
            fig, ax = plt.subplots(figsize=(6,6))
            plot_ellipse(a_ell_h, b_ell_h, h = org_hyp_h*2 + org_ell_h, ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='y')
            plot_hyperbola(a_hyp_h, b_hyp_h, h = org_hyp_h, orientation='horizontal',tmax=0.2, linewidth=0.5, ax=ax, label='Hyperbola', linestyle='--',color='y')
            ax.scatter([org_hyp_h*2,org_hyp_h*2+org_ell_h*2], [0,0], color='red',s=0.2)  # 焦点
            ax.set_xlim([145.8, 146.9])
            ax.set_ylim([-0.55, 0.55])
            plt.savefig('fig_wolter3_H.png', dpi=300,transparent=True)
            plt.show()
...
def compare_sep(rays, points,coeffs_det0,ray_num,region):
    ...
    num = 10000
    if widesearch:
        x_min = coeffs_det0[9] - 1e-1
        x_max = coeffs_det0[9] + 1e-1
    else:
...
def KB_debug(params,na_ratio_h,na_ratio_v,option,option_legendre=False,source_shift=[0.,0.,0.],option_save=True,designparams=None):
    defocus, astigH, \
    pitch_hyp_v, roll_hyp_v, yaw_hyp_v, decenterX_hyp_v, decenterY_hyp_v, decenterZ_hyp_v,\
    pitch_hyp_h, roll_hyp_h, yaw_hyp_h, decenterX_hyp_h, decenterY_hyp_h, decenterZ_hyp_h,\
    pitch_ell_v, roll_ell_v, yaw_ell_v, decenterX_ell_v, decenterY_ell_v, decenterZ_ell_v,\
    ...
    if optKBdesign:
        ...
        if option_HighNA == True:
            if designparams is None:
                # 論文用　あえてでかい
                if option_energy == 'EUV':
                    l1h, l2h, inc_h, mlen_h, wd_v, inc_v, mlen_v = [np.float64(146.), np.float64(0.57),  np.float64(0.2132), np.float64(0.4), np.float64(0.13), np.float64(0.2132), np.float64(0.14)]
                elif option_energy == 'hardXray':
                    l1h, l2h, inc_h, mlen_h, wd_v, inc_v, mlen_v = [np.float64(146.), np.float64(0.57),  np.float64(0.2132/44), np.float64(0.4), np.float64(0.13), np.float64(0.2132/44), np.float64(0.14)]
                elif option_energy == 'softXray':
                    ### 1000eV 25mrad AL coat
                    ### 1000eV 25mrad AL coat
                    l1h, l2h, inc_h, mlen_h, wd_v, inc_v, mlen_v = [np.float64(146.), np.float64(0.57),  np.float64(0.2132/10), np.float64(0.4), np.float64(0.13), np.float64(0.2132/10), np.float64(0.14)]
                ...
                # elif option_energy == 'hardXray':
                #     l1h, l2h, inc_h, mlen_h, wd_v, inc_v, mlen_v = [np.float64(146.), np.float64(0.43),  np.float64(0.213/8.2), np.float64(0.30), np.float64(0.1), np.float64(0.175/8.2), np.float64(0.14)]
    ...
    if False:
        ...
        fig, ax = plt.subplots(figsize=(6,6))
        plot_ellipse(a_hyp_v, b_hyp_v, h = org_hyp_v , ax=ax, label='Ellipse', linewidth=0.5, linestyle='--',color='y')
        # plot_hyperbola(a_hyp_v, b_hyp_v, h = org_hyp_v, orientation='horizontal',tmax=0.2, linewidth=0.5, ax=ax, label='Hyperbola', linestyle='--',color='y')
        ax.scatter([org_hyp_v*2], [0], color='red',s=0.2)  # 焦点
        ax.set_xlim([145.8, 146.3])
        ...
        # plot_hyperbola(a_hyp_v, b_hyp_v, h = org_hyp_v, orientation='horizontal',tmax=0.2, linewidth=0.5, ax=ax, label='Hyperbola', linestyle='--',color='y')
        ax.scatter([org_hyp_h*2], [0], color='red',s=0.2)  # 焦点
        ax.set_xlim([145.8, 146.3])
        ax.set_ylim([-0.25, 0.25])
        plt.savefig('fig_ellipse_H.png', dpi=300,transparent=True)
        plt.show()
    ...
    if bool_point_source:
        ...
        if option == 'sep' or option == 'wave' or option == 'ray':
            ...
            if option == 'ray':
                if False:
                    ...
                    #     source_edge = vmirr_hyp - (vmirr_hyp - source)/(vmirr_hyp[0,0] - source[0,0]) *900e-3
                    #     ax.plot([source_edge[0,i], vmirr_hyp[0,i]], [source_edge[1,i], vmirr_hyp[1,i]], [source_edge[2,i], vmirr_hyp[2,i]], color='#271558', alpha=0.3,linewidth=1)
                    # # 光線2nd to 3rd
                    # for i in range(0, ray_num**2, round(ray_num**2/40)):
                    #     ax.plot([hmirr_hyp[0,i], vmirr_hyp[0,i]], [hmirr_hyp[1,i], vmirr_hyp[1,i]], [hmirr_hyp[2,i], vmirr_hyp[2,i]], color="#271558", alpha=0.3,linewidth=1)
                    ...
                    # # for i in range(0, ray_num**2, round(ray_num**2/20)):
                    # #     ax.plot([hmirr_ell[0,i], vmirr_ell[0,i]], [hmirr_ell[1,i], vmirr_ell[1,i]], [hmirr_ell[2,i], vmirr_ell[2,i]], color='#271558', alpha=0.3,linewidth=1)
                    # detcenter_edge = hmirr_hyp + (detcenter - hmirr_hyp) *900e-1
                    # for i in range(0, ray_num**2, round(ray_num**2/40)):
                    #     ax.plot([hmirr_hyp[0,i], detcenter_edge[0,i]], [hmirr_hyp[1,i], detcenter_edge[1,i]], [hmirr_hyp[2,i], detcenter_edge[2,i]], color='#271558', alpha=0.3,linewidth=1)
        ...
        if option == 'ray_wave':
            ...
            if option_save:
                ...
                plt.figure()
                plt.imshow(rectified_img, cmap='jet',vmin = -1/16,vmax = 1/16)
                plt.title(f'PV 6σ={np.nanstd(matrixWave2_Corrected/lambda_)*6}')
                plt.savefig(os.path.join(directory_name,'rectified_img3.png'), transparent=True, dpi=300)
        ...
        if option == 'ray':
            ...
            if False:
                min_Y = np.min(detcenter[1, :]) - 3e-9
                max_Y = np.max(detcenter[1, :]) + 3e-9
                min_Z = np.min(detcenter[2, :]) - 3e-9
                max_Z = np.max(detcenter[2, :]) + 3e-9
                ### defocus each plot +- defocuss
                defocussize = 0.3e-8
                std1 = []
                std2 = []
                std3 = []
                ...
                std5 = []
                std6 = []
                def calc_rms_radius(x, y, center=None):
                    """
                    スポットダイアグラムの RMS 半径を計算する。
                    ...
                    center : tuple of float, optional
                        重心の代わりに使う中心座標 (x0, y0)。指定がない場合は重心を使用。
                    ...
                    rms_radius = np.sqrt(np.mean(r_squared))

                    return rms_radius
                for i in range(13):
                    defocus_shift = defocussize * (i - 5)
                    coeffs_det3 = np.zeros(10)
                    coeffs_det3[6] = 1
                    coeffs_det3[9] = -(s2f_middle + defocus + defocus_shift)
                    detcenter3 = plane_ray_intersection(coeffs_det3, reflect2_rotated, hmirr_hyp_points_rotated)
                    fig0, axs0 = plt.subplots(3, 2, figsize=(10, 15))
                    fig0.suptitle(f'Defocus shift: {defocus_shift*1e9:.2f} nm', fontsize=16)
                    matrix_detcenterY = detcenter3[1, :].reshape(ray_num_V, ray_num_H)
                    matrix_detcenterZ = detcenter3[2, :].reshape(ray_num_V, ray_num_H)
                    ### Z方向1/6ずつ
                    third = ray_num_V // 6
                    axs0[0, 0].scatter(matrix_detcenterY[:third, :], matrix_detcenterZ[:third, :])
                    std1_local = calc_rms_radius(matrix_detcenterY[:third, :], matrix_detcenterZ[:third, :])
                    std1.append(std1_local)
                    axs0[0, 0].set_title('Z direction 0-1/6, rms={:.2f}nm'.format(std1_local*1e9))
                    ### 真ん中 Z方向1/6
                    ...
                    std3.append(std3_local)
                    axs0[2, 0].set_title('Z direction 5/6-1, rms={:.2f}nm'.format(std3_local*1e9))
                    axs0[0, 1].scatter(matrix_detcenterY[:, :third], matrix_detcenterZ[:, :third])
                    std4_local = (calc_rms_radius(matrix_detcenterY[:, :third], matrix_detcenterZ[:, :third]))
                    std4.append(std4_local)
                    axs0[0, 1].set_title('Y direction 0-1/6, rms={:.2f}nm'.format(std4_local*1e9))
                    axs0[1, 1].scatter(matrix_detcenterY[:, third*5//2:third*7//2], matrix_detcenterZ[:, third*5//2:third*7//2])
                    std5_local = (calc_rms_radius(matrix_detcenterY[:, third*5//2:third*7//2], matrix_detcenterZ[:, third*5//2:third*7//2]))
                    std5.append(std5_local)
                    axs0[1, 1].set_title('Y direction 5/12-7/12, rms={:.2f}nm'.format(std5_local*1e9))
                    axs0[2, 1].scatter(matrix_detcenterY[:, third*5:], matrix_detcenterZ[:, third*5:])
                    ...
                    plt.tight_layout()
                    plt.savefig(os.path.join(directory_name, f'defocus_shift_{defocus_shift*1e9:.2f}nm.png'), transparent=True, dpi=300)
                    plt.close(fig0)
                plt.figure()
                x_here = defocussize*(np.arange(13) - 5)
                plt.plot(x_here, std1, label='Z direction 0-1/6')
                plt.plot(x_here, std2, label='Z direction 5/12-7/12')
                plt.plot(x_here, std3, label='Z direction 5/6-1')
                plt.plot(x_here, std4, label='Y direction 0-1/6')
                plt.plot(x_here, std5, label='Y direction 5/12-7/12')
            ...
            #     third = ray_num_V // 3
            #     axs0[0, 0].scatter(matrix_detcenterY[:third, :], matrix_detcenterZ[:third, :])
            #     axs0[0, 0].set_title('Z direction 1/3')
            #     axs0[1, 0].scatter(matrix_detcenterY[third:2*third, :], matrix_detcenterZ[third:2*third, :])
            #     axs0[1, 0].set_title('Z direction 2/3')
            #     axs0[2, 0].scatter(matrix_detcenterY[2*third:, :], matrix_detcenterZ[2*third:, :])
            #     axs0[2, 0].set_title('Z direction 3/3')
            #     axs0[0, 1].scatter(matrix_detcenterY[:, :third], matrix_detcenterZ[:, :third])
            #     axs0[0, 1].set_title('Y direction 1/3')
            #     axs0[1, 1].scatter(matrix_detcenterY[:, third:2*third], matrix_detcenterZ[:, third:2*third])
            #     axs0[1, 1].set_title('Y direction 2/3')
            #     axs0[2, 1].scatter(matrix_detcenterY[:, 2*third:], matrix_detcenterZ[:, 2*third:])
            #     axs0[2, 1].set_title('Y direction 3/3')
            #     for ax in axs0.flat:
            #         ax.set_xlim(min_Y, max_Y)
            #         ax.set_ylim(min_Z, max_Z)
            #     plt.tight_layout()
            # plt.show()
            # ### defocus each plot +- defocuss
            # min_Y = np.min(detcenter[1, :]) - 2e-9
            # max_Y = np.max(detcenter[1, :]) + 2e-9
            ...
            #     coeffs_det3[9] = -(s2f_middle + defocus + defocus_shift)
            #     detcenter3 = plane_ray_intersection(coeffs_det3, reflect2_rotated, hmirr_hyp_points_rotated)
            #     fig0, axs0 = plt.subplots(3, 3, figsize=(15, 15))
            #     fig0.suptitle(f'Defocus shift: {defocus_shift*1e9:.2f} nm', fontsize=16)
            #     matrix_detcenterY = detcenter3[1, :].reshape(ray_num_V, ray_num_H)
            #     matrix_detcenterZ = detcenter3[2, :].reshape(ray_num_V, ray_num_H)
            #     ### Z方向1/3ずつ
            ...
            #     axs0[0, 1].set_title('Z direction 5/12-7/12, Y direction 0-1/6')
            #     axs0[0, 2].scatter(matrix_detcenterY[third*5:, :third], matrix_detcenterZ[third*5:, :third])
            #     axs0[0, 2].set_title('Z direction 5/6-1, Y direction 0-1/6')
            #     axs0[1, 0].scatter(matrix_detcenterY[:third, third*5//2:third*7//2], matrix_detcenterZ[:third, third*5//2:third*7//2])
            #     axs0[1, 0].set_title('Z direction 0-1/6, Y direction 5/12-7/12')
            #     axs0[1, 1].scatter(matrix_detcenterY[third*5// 2:third*7//2, third*5//2:third*7//2], matrix_detcenterZ[third*5//2:third*7//2, third*5//2:third*7//2])
            #     axs0[1, 1].set_title('Z direction 5/12-7/12, Y direction 5/12-7/12')
            #     axs0[1, 2].scatter(matrix_detcenterY[third*5:, third*5//2:third*7//2], matrix_detcenterZ[third*5:, third*5//2:third*7//2])
            #     axs0[1, 2].set_title('Z direction 5/6-1, Y direction 5/12-7/12')
            #     axs0[2, 0].scatter(matrix_detcenterY[:third, third*5:], matrix_detcenterZ[:third, third*5:])
            #     axs0[2, 0].set_title('Z direction 0-1/6, Y direction 5/6-1')
            #     axs0[2, 1].scatter(matrix_detcenterY[third*5//2:third*7//2, third*5:], matrix_detcenterZ[third*5//2:third*7//2, third*5:])
            #     axs0[2, 1].set_title('Z direction 5/12-7/12, Y direction 5/6-1')
            #     axs0[2, 2].scatter(matrix_detcenterY[third*5:, third*5:], matrix_detcenterZ[third*5:, third*5:])
            #     axs0[2, 2].set_title('Z direction 5/6-1, Y direction 5/6-1')
            #     for ax in axs0.flat:
            #         ax.set_xlim(min_Y, max_Y)
            #         ax.set_ylim(min_Z, max_Z)
            #     plt.tight_layout()
            ...
            #     axs0[0, 1].set_title('Z direction 1/3, Y direction 2/3')
            #     axs0[0, 2].scatter(matrix_detcenterY[:third, 2*third:], matrix_detcenterZ[:third, 2*third:])
            #     axs0[0, 2].set_title('Z direction 1/3, Y direction 3/3')
            #     axs0[1, 0].scatter(matrix_detcenterY[third:2*third, :third], matrix_detcenterZ[third:2*third, :third])
            #     axs0[1, 0].set_title('Z direction 2/3, Y direction 1/3')
            #     axs0[1, 1].scatter(matrix_detcenterY[third:2*third, third:2*third], matrix_detcenterZ[third:2*third, third:2*third])
            #     axs0[1, 1].set_title('Z direction 2/3, Y direction 2/3')
            #     axs0[1, 2].scatter(matrix_detcenterY[third:2*third, 2*third:], matrix_detcenterZ[third:2*third, 2*third:])
            #     axs0[1, 2].set_title('Z direction 2/3, Y direction 3/3')
            #     axs0[2, 0].scatter(matrix_detcenterY[2*third:, :third], matrix_detcenterZ[2*third:, :third])
            #     axs0[2, 0].set_title('Z direction 3/3, Y direction 1/3')
            #     axs0[2, 1].scatter(matrix_detcenterY[2*third:, third:2*third], matrix_detcenterZ[2*third:, third:2*third])
            #     axs0[2, 1].set_title('Z direction 3/3, Y direction 2/3')
            #     axs0[2, 2].scatter(matrix_detcenterY[2*third:, 2*third:], matrix_detcenterZ[2*third:, 2*third:])
            #     axs0[2, 2].set_title('Z direction 3/3, Y direction 3/3')
            #     for ax in axs0.flat:
            #         ax.set_xlim(min_Y, max_Y)
            #         ax.set_ylim(min_Z, max_Z)
            #     plt.tight_layout()
...
def auto_focus_NA(num_adj_astg,initial_params,na_ratio_h,na_ratio_v,option,option_param,option_disp='ray',option_mode=False,source_shift0=[0.,0.,0.],option_legendre=False):
    if widesearch:
        initial_a_min = -1 + initial_params[0].copy()  # Initial minimum value for 'a'
        initial_a_max = 1 + initial_params[0].copy() # Initial maximum value for 'a'
        shrink_factor = 0.1    # Range reduction factor per loop
        num_adj_astg = 300      # Number of steps for each range
        max_attempts = 17     # Maximum attempts to avoid infinite loop
    else:
...
def LegendrealignmentKB(initial_params, num_param, range_param,tuning=True):
    global option_mpmath
    # Legendre alignment
    innerproducts = []
    pvs = []
    ...
    for i in range(col):
        ...
        if False:
            ...
            plt.plot(range_param, pvs[:,i], label=f'legendre {order[i]}', color=colors[i])
            # plt.ylim(np.min(pvs), np.max(pvs))
            ### linear fit
            coeffs = np.polyfit(range_param, pvs[:,i], 1)
        fit_line = np.polyval(coeffs, range_param)
        plt.plot(range_param, fit_line, '--', label=f'y={coeffs[0]:.2e}x + {coeffs[1]:.2e}', color=colors[i])
...
if option_AKB == False:
    ratio = (np.arange(1)+1)
    # ratio = np.linspace(0.001, 0.4, 20)
